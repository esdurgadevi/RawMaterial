...
const Broker = () => {
  ...
  // Form state - Updated field names to match brokerService
  ...
  const fetchBrokers = async () => {
    ...
    try {
      ...
      // Extract brokers array from response
  ...
  const filteredBrokers = (() => {
    ...
    return brokersArray.filter(broker => {
      ...
      const brokerCode = broker.brokerCode ? broker.brokerCode.toString() : '';
  ...
  const handleInputChange = (e) => {
    ...
    setFormData(prev => ({
      ...prev,
      [name]: name === 'brokerCode' ? (value === '' ? '' : parseInt(value) || value) : value
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingBroker) {
        ...
        await brokerService.update(editingBroker.id, formData);
        setSuccess('Broker updated successfully!');
      } else {
        // Create new broker
        await brokerService.create(formData);
  ...
  const handleDelete = async (id, brokerName) => {
    if (!id || !brokerName) {
      setError('Invalid broker data for deletion');
      return;
    }
  ...
  const exportBrokers = async () => {
    try {
      const csvContent = "data:text/csv;charset=utf-8," +
        "Code,Broker Name,Short Description,Address\n" +
        filteredBrokers.map(b => 
          `${b.brokerCode},${b.brokerName},${b.shortDesc || ''},${b.address || ''}`
        ).join("\n");
      ...
      setSuccess('Brokers exported successfully!');
      setTimeout(() => setSuccess(''), 3000);
    } catch (err) {
      setError('Failed to export brokers');
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      ...
      <div className="bg-white rounded-lg shadow-sm p-4 md:p-6 mb-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-gray-800 mb-2">Broker Management</h1>
            <p className="text-gray-600">Manage all brokers and their information</p>
          </div>
          <button
        ...
        <div className="flex flex-col md:flex-row md:items-center space-y-4 md:space-y-0 md:space-x-4">
          ...
          <div className="flex-1">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search brokers by code, name, or description..."
                value={searchTerm}
      ...
      <div className="bg-white rounded-lg shadow-sm overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="p-8 text-center">
            ...
            <p className="text-gray-600 mb-2">No brokers found</p>
            {searchTerm ? (
                  ...
                  <tr key={broker.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex space-x-2">
                        ...
                        <button
                          onClick={() => handleDelete(broker.id, broker.brokerName)}
              ...
              <form onSubmit={handleSubmit}>
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    {editingBroker && (
                      <p className="mt-1 text-xs text-gray-500">Broker code cannot be changed after creation</p>
                    )}
                  </div>
                  ...
                  {/* Display existing broker info when editing */}
                  {editingBroker && (
                    <div className="p-3 bg-gray-50 rounded-lg border border-gray-200">
                      <h4 className="text-sm font-medium text-gray-700 mb-2">Broker Information</h4>
                      <div className="grid grid-cols-2 gap-2 text-sm">
                        <div>
                          <span className="text-gray-500">Created:</span>
                          <div className="font-medium">
                            {editingBroker.createdAt ? new Date(editingBroker.createdAt).toLocaleString() : 'N/A'}
                          </div>
                        </div>
                        <div>
                          <span className="text-gray-500">Last Updated:</span>
                          <div className="font-medium">
                            {editingBroker.updatedAt ? new Date(editingBroker.updatedAt).toLocaleString() : 'N/A'}
                          </div>
                        </div>
                      </div>
                    </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              {/* Modal Header */}
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-xl font-semibold text-gray-800">Broker Details</h3>
                <button
              ...
              {/* Broker Details - Updated field names */}
              <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="mt-1 flex items-center">
                        ...
                        <span className="font-mono font-semibold text-gray-900">
                          #{formatCode(viewingBroker.brokerCode)}
                        </span>
                      </div>
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-500 uppercase">ID</label>
                      <div className="mt-1 text-sm text-gray-900">{viewingBroker.id}</div>
                    </div>
                  </div>

                  <div>
                    <label className="block text-xs font-medium text-gray-500 uppercase">Broker Name</label>
                    <div className="mt-1 flex items-center">
                      <Building2 className="w-4 h-4 text-gray-400 mr-2" />
                      <span className="text-lg font-medium text-gray-900">{viewingBroker.brokerName}</span>
                    </div>
                  </div>

                  {viewingBroker.shortDesc && (
                    <div>
                      ...
                      <div className="mt-1 p-3 bg-gray-50 rounded-lg border border-gray-200">
                        <div className="flex items-start">
                          ...
                          <div className="text-sm text-gray-900">{viewingBroker.shortDesc}</div>
                        </div>
                      </div>
                    </div>
                  )}
                    ...
                    <div>
                      ...
                      <div className="mt-1 p-3 bg-gray-50 rounded-lg border border-gray-200">
                        <div className="flex items-start">
                          ...
                          <div className="text-sm text-gray-900 whitespace-pre-line">{viewingBroker.address}</div>
                        </div>
                      </div>
                    </div>
                  )}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <div className="mt-1 text-sm font-medium text-gray-900">
                          {formatDate(viewingBroker.createdAt)}
                        </div>
                        <div className="text-xs text-gray-500">
                          {formatTime(viewingBroker.createdAt)}
                        </div>
                      </div>
                      <div>
                        <label className="block text-xs text-gray-500">Last Updated</label>
                        <div className="mt-1 text-sm font-medium text-gray-900">
                          {formatDate(viewingBroker.updatedAt)}
                        </div>
                        <div className="text-xs text-gray-500">
                          {formatTime(viewingBroker.updatedAt)}
                        </div>
                      </div>
                    </div>
                  </div>
